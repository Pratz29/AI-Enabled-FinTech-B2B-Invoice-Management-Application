	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	   ...
	   if (customerOrderIdParam != null && !customerOrderIdParam.isEmpty()) {
        ...
        try { 
			...
			if(result == 1) {
				res = "Record entered successfully!No of rows inserted is 1";
				out.println("Record entered successfully!No of rows inserted is 1");
			}
			else {
				res = "Record enter unsuccessfully!No of rows inserted is 0";
				out.println("Record enter unsuccessfully!No of rows inserted is 0");
			}
			
			List<Invoice> insertedRecord = inv.getInvoice(Sl_no,3);
			String jsonInsertedRecord = gson.toJson(insertedRecord);
			
			out.println("Inserted record:");
            out.println(jsonInsertedRecord);
